...
class AddressCarouselState extends State<AddressCarousel> {
  final int availableMode = getAvailableAddrs(coin: aa.coin, id: aa.id);
...
class _QRAddressState extends State<QRAddressWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return Observer(builder: (context) {
      ...
      final uri = a != 0 || widget.memo?.isNotEmpty == true
          ? makePaymentUri(
              coin: aa.coin, address: address, amount: BigInt.from(widget.amount ?? 0), memo: widget.memo ?? '')
          : address;
  ...
  String get address {
    ...
    return getAddress(coin: aa.coin, id: aa.id, ua: uaType);
  }
  ...
  String get uri {
    ...
    final uri = a != 0
        ? makePaymentUri(
            coin: aa.coin, address: address, amount: BigInt.from(widget.amount ?? 0), 
            memo: widget.memo ?? '')
        : address;
    return uri;
  }
